$(function(){
    //◆ 共通
        var domain ="blog.sawasawaworks.com", //記事を取得したいtumblrブログのドメイン
            limit = "5", //取得したい記事数
            api_key = "xq2COMO7SeFjKSapb2VRKTfnm8FGeduoHspOB13aGYLN9rJSsK";//apikey

    //◆対象URLの投稿IDを取得する
    function get_postId(obj_url) {
        var my_id_index = obj_url.indexOf("post/")+5,
            my_id_str = obj_url.substr(my_id_index),
            my_id_end = my_id_str.indexOf("/"),
            my_id = my_id_str.substring(0 ,my_id_end);
        return my_id;
    }
    //◆テキスト投稿に含まれる最初の画像のURLを取得する
    function get_postTxtImage(postTxt_body) {
        var
            img_index = postTxt_body.indexOf("<img"),
            img_str1 = postTxt_body.substr(img_index),
